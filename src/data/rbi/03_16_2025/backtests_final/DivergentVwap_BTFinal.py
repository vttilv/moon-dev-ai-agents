...
class DivergentVwap(Strategy):
    ...
    def init(self):
        ...
        self.stoch_k = self.I(lambda: talib.STOCH(
            self.data.High, self.data.Low, self.data.Close,
            fastk_period=self.stoch_period, slowk_period=3,
            slowk_matype=0, slowd_period=3, slowd_matype=0)[0])
            
        self.stoch_d = self.I(lambda: talib.STOCH(
            self.data.High, self.data.Low, self.data.Close,
            fastk_period=self.stoch_period, slowk_period=3,
            slowk_matype=0, slowd_period=3, slowd_matype=0)[1])
        
        self.vwap = self.I(lambda: pta.vwap(
