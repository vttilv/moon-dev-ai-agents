...
class VolSurgeBreakout(Strategy):
    ...
    def next(self):
        ...
        if not self.position and len(self.data.Close) > 3:  # Changed >= to > for proper indexing
            ...
            if in_compression and volume_spike and breakout_trigger:
                ...
                if position_size > 0:
                    ...
                    print(f"🌙🚀 MOON BLASTOFF! Long {position_size} units @ ${entry_price:.2f} | Stop: ${stop_loss:.2f}")
...
# LAUNCH MOON MISSION 🌙🚀
bt = Backtest(data, VolSurgeBreakout, cash=1_000_000, commission=0.002)  # Fixed decimal format
stats = bt.run()
print("\n🌕🌖🌗🌘🌑🌒🌓🌔🌕")
