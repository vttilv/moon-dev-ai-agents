            ...
            if (self.ema50[-2] < self.ema200[-2] and self.ema50[-1] > self.ema200[-1]) and self.adx[-1] < 20:
                ...
                if risk_per_unit > 0:
                    ...
                    if position_size > 0:
                        ...
                        print(f"🌙✨ GOLDEN CROSS ALERT! Entry: {price:.2f} | Size: {position_size} units | Risk: ${risk_per_unit:.2f} 🚀")
...
bt = Backtest(data, AdaptiveCrossover, cash=1_000_000, commission=.002, exclusive_orders=True)
