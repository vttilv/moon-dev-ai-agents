...
from pandas_ta import fibonacci, macd as ta_macd, signal, hist
...
class DynamicBreakoutWithinFib(Strategy):
    ...
    def __init__(self):
        ...
        self.macd, self.signal_line, self_histograms = ta_macd(
    ...
    def next(self):
        if self.position:
            ...
            lot_size = int(risk * 2)  # Removed rounding and cast to integer
            
            if lot_size > 0:
                self.position.close()
                self.position.size = lot_size  # Simplified assignment
                print(f"Closed position with size: {lot_size}")
                self.position.open()
