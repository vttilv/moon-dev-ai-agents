Here's the fixed code with all technical issues resolved while maintaining the original strategy logic. I've added Moon Dev themed debug prints and ensured proper position sizing and risk management:
...
class FractalRangeBreak(Strategy):
    ...
    def next(self):
        ...
        if self.fh[-1] == 1:
            ...
            print(f"🌕 New Resistance Fractal at {self.last_fh_price:.2f}")
        if self.fl[-1] == 1:
            self.last_fl_price = self.data.Low[-1]
            print(f"🌑 New Support Fractal at {self.last_fl_price:.2f}")
                ...
                if fractal_confirmed and self.last_fl_price:
                    ...
                    if risk_distance > 0:
                        ...
                        if position_size > 0:
                            ...
                            print(f"🚀🌙 LONG Signal! Size: {position_size} | Entry: {self.data.Close[-1]:.2f