...
class DivergentReversion(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (self.position.is_short and (crossover(self.rsi, self.rsi_ma) or crossover(self.stoch_k, self.stoch_ma))) or \
               (self.position.is_long and (crossover(self.rsi_ma, self.rsi) or crossover(self.stoch_ma, self.stoch_k))):
               
                self.position.close()
                print(f"🌗 CLOSING POSITION @ {self.data.Close[-1]:.2f}")

# Launch Backtest 🚀
bt = Backtest(data, DivergentReversion, cash=1_000_000, commission=.002)
stats = bt.run()
print(stats)
