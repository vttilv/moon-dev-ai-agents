...
class MeanReversalAlert(Strategy):
    ...
    def init(self):
        ...
        def bollinger_bands(close):
            return talib.BBANDS(close, 20, 2, 2, 0)[0], talib.BBANDS(close, 20, 2, 2, 0)[2]
        
        self.upper_bb = self.I(bollinger_bands, self.data.Close, name='Upper BB')
        self.lower_bb = self.I(bollinger_bands, self.data.Close, name='Lower BB')
        
        # 📏 Volatility Measurement
        def atr(close):
            return talib.ATR(close, 14)
        
        self.atr = self.I(atr, close=self.data.Close, timeperiod=14, name='ATR 14')
