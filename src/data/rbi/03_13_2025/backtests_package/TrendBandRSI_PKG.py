...
class TrendBandRSI(Strategy):
    ...
    def next(self):
        ...
        current_upper = self.sma_rsi[-1] + self.std_rsi[-1]
        previous_upper = self.sma_rsi[-2] + self.std_rsi[-2]
        current_lower = self.sma_rsi[-1] - self.std_rsi[-1]
        previous_lower = self.sma_rsi[-2] - self.std_rsi[-2]
