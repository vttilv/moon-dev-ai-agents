                ...
                if current_volume > self.volume_spike * avg_volume and avg_volume > 0:
                    ...
                    if risk_per_share > 0:
                        ...
                        if position_size > 0:
                            ...
                            print(f"🚀🌕 MOON DEV ENTRY SIGNAL! | Size: {position_size} units | Entry: {entry_price:.2f} | SL: {stop_loss:.2f}")

        # Cosmic Exit Logic 🌠
        elif self.rsi[-1] >= 70:
            exit_price = self.data.Close[-1]
            self.position.close()
            print(f"🌑✨ MOON DEV EXIT SIGNAL! | Price: {exit_price:.2f} | RSI: {self.rsi[-1]:.2f}")

# ======== BACKTEST EXECUTION ========
# Initialize lunar backtest with proper parameters 🌕
bt = Backtest(data, VolumetricCross, cash=1000000, commission=.002, exclusive_orders=True)
