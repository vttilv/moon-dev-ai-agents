...
class VortexEmergence(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (self.vi_plus[-2] < self.vi_minus[-2] and self.vi_plus[-1] > self.vi_minus[-1]) and (self.choppiness[-1] < 35):
        ...
        # Galactic Exit Protocol 🌌
            ...
            if self.choppiness[-1] > 65:
                self.position.close()
                print(f"✨🌙 STARLIGHT RETREAT! Exit: {price:.2f} | Profit: {self.position.pl_pct:.2%}")
