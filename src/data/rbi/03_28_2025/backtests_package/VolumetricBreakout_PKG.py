...
class VolumetricBreakout(Strategy):
    ...
    def init(self):
        ...
        self.volume_pct = self.I(ta.percentile, self.data.Volume, length=20, percentile=90)
    ...
    def next(self):
        ...
        if not self.position and price > upper and volume > vol_pct:
            ...
            if position_size > 0:
                ...
                print(f"🚀 BLASTOFF! Long {position_size} @ {entry_price:.2f} | Stop: {final_stop:.2f} 🌕")

        # 🌌 Exit Logic
        if self.position:
            rsi_value = self.rsi[-1]
            # Replaced crossover with direct comparison
            if (self.rsi[-2] < 70 and self.rsi[-1] >= 70) or price < self.upper_band[-1]:
                self.position.close()
