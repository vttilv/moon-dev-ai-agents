        ...
        if not self.position:
            ...
            if close_cross and self.adx[-1] > 25:
                ...
                if vol_ratio >= 1.2:
                    ...
                    if position_size > 0:
                        print(f"🌙🚀 MOON ENTRY: Size {position_size} @ {self.data.Close[-1]:.2f} | SL: {sl:.2f} | TP: {tp:.2f} ✨")
                        self.buy(size=position_size, sl=sl, tp=tp)

bt = Backtest(data, VolatilitySurge, cash=1_000_000, commission=.002, exclusive_orders=True)
stats = bt.run()
