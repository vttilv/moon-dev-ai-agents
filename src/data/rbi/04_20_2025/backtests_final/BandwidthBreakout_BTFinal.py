...
class BandwidthBreakout(Strategy):
    def init(self):
        ...
        self.bandwidth = self.I(lambda u, l: (u - l)/self.I(talib.SMA, self.data.Close, timeperiod=20), 
            ...
            if bw == bw_low:
                if price > ema:
                    ...
                    if size > 0:
                        ...
                        print(f"🌙🚀 MOON SHOT! LONG ENTRY @ {price:.2f} | Size: {size} | ATR Risk: {atr:.2f} | SL: {sl:.2f} | TP: {tp:.2f} 🌕")
                elif price < ema:
                    risk = 0.01 * self.equity
                    size = int(round(risk / (atr * 1)))
                    if size > 0:
                        ...
                        print(f"🌙💫 STAR GAZER! SHORT ENTRY @ {price:.2f} | Size: {size} | ATR Risk: {atr:.2f} | SL: {sl:.2f} | TP: {tp:.2f} 🌠")
