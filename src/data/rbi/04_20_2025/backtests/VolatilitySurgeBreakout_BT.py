...
class VolatilitySurgeBreakout(Strategy):
    def init(self):
        self.atr_5m = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, 14, name='ATR_5m')
        self.atr_daily = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, 96, name='ATR_Daily')
