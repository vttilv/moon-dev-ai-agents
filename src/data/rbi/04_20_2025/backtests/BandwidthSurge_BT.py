...
class BandwidthSurge(Strategy):
    def init(self):
        ...
        self.upper = self.I(talib.BBANDS, self.data.Close, 20, 2, 2, name='Upper')
        self.middle = self.I(talib.BBANDS, self.data.Close, 20, 2, 2, name='Middle', which=1)
        self.lower = self.I(talib.BBANDS, self.data.Close, 20, 2, 2, name='Lower', which=2)
        
        # Bollinger Bandwidth calculation
        self.bbw = self.I(lambda u, l, m: (u - l) / m, self.upper, self.lower, self.middle)
        self.bbw_low = self.I(talib.MIN, self.bbw, 20)
        
        # Volume indicators
        self.volume_ma50 = self.I(talib.SMA, self.data.Volume, 50)
