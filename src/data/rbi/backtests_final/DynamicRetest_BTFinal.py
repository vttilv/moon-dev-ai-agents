    ...
    def next(self):
        ...
        if trend == 'up' and current_close > current_open:
            ...
            if curr_zone_bottom < current_close <= curr_zone_top:
                ...
                # Round position size to whole number (backtesting requirement)
                position_size = int(round(position_size))
                if position_size < 1:
                    print("🚨 Moon Dev: Position size too small (< 1 unit). Skipping trade!")
                    return
                
                # Set stop loss just below the demand zone (a slight buffer)
                stop_loss = curr_zone_bottom * 0.999

                print(f"🌙🚀 Moon Dev: LONG signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size} units")
        ...
        if trend == 'down' and current_close < current_open:
            ...
            if curr_zone_bottom <= current_close < curr_zone_top:
                ...
                print(f"🌙🚀 Moon Dev: SHORT signal detected! Entry = {current_close:.2f}, Stop Loss = {stop_loss:.2f}, Take Profit = {take_profit:.2f}, Size = {position_size} units")
