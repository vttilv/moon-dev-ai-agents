...
class CoTrendalNeutral(Strategy):
    ...
    def next(self):
        ...
        if (np.isnan(self.sma50[-1]) or np.isnan(self.sma200[-1]) or 
            np.isnan(self.atr[-1]) or np.isnan(self.atr_ma[-1]) or
            np.isnan(self.swing_high[-1]) or np.isnan(self.swing_low[-1])):
            return
