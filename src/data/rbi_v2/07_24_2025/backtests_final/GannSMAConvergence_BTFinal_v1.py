...
class GannSMAConvergence(Strategy):
    def init(self):
        ...
        cols_to_drop = [col for col in self.data.df.columns if 'unnamed' in col.lower()]
        if cols_to_drop:
            self.data.df = self.data.df.drop(columns=cols_to_drop)
        ...
        self.sma5 = self.I(talib.SMA, self.data.df['Close'], timeperiod=5)
        self.atr14 = self.I(talib.ATR, self.data.df['High'], self.data.df['Low'], self.data.df['Close'], timeperiod=14)
        
        # Gann angle calculations (simplified)
        self.swing_high = self.I(talib.MAX, self.data.df['High'], timeperiod=20)
        self.swing_low = self.I(talib.MIN, self.data.df['Low'], timeperiod=20)
...
bt = Backtest(data, GannSMAConvergence, commission=.002, margin=1.0, trade_on_close=True, cash=100000)
stats = bt.run()
print(stats)
