...
class VolumePulseBreakout(Strategy):
    ...
    def next(self):
        ...
        previous_close = self.data.Close[-2] if len(self.data.Close) > 1 else current_close
        ...
        # Entry conditions
        entry_conditions = (
            len(self.data.Close) > 1 and  # Ensure we have enough data
            current_close < self.bb_lower[-1] and  # Price was below lower BB
            self.rsi[-1] < 30 and  # RSI was oversold
            current_volume > 1.5 * self.vwma[-1] and  # Volume spike
