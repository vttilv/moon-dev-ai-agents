...
class BandedStochastic(Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            if (price > upper and 
                (self.slowk[-1] < self.slowd[-1] and self.slowk[-2] > self.slowd[-2]) and 
                ...
                if position_size > 0:
                    print(f"🌙 MOON DEV LONG SIGNAL DETECTED! ✨ Price: {price:.2f}, Upper Band: {upper:.2f}, Stoch K: {slowk:.2f}, Stoch D: {slowd:.2f}")
                ...
                if position_size > 0:
                    print(f"🌙 MOON DEV SHORT SIGNAL DETECTED! ✨ Price: {price:.2f}, Lower Band: {lower:.2f}, Stoch K: {slowk:.2f}, Stoch D: {slowd:.2f}")
