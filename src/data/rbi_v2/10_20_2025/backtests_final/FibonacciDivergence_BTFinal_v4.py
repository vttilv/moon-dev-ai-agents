...
class FibonacciDivergence(Strategy):
    ...
    def is_bullish_divergence(self):
        if self._i < self.prev_lookback:
            return False
        start_prev = max(0, self._i + 1 - self.prev_lookback)
        end_prev = max(0, self._i + 1 - self.lookback_div)
        start_recent = end_prev
        end_recent = self._i + 1
    ...
    def next(self):
        ...
        current_bar = self._i
        close = self.data.Close[self._i]
        high = self.data.High[self._i]
        low = self.data.Low[self._i]

        uptrend = close > self.ema200[self._i]
        downtrend = close < self.ema200[self._i]
